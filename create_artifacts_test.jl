...
function apply_W_beam(map::HealpixMap{T,O}, lmax::Int, PSF_theta::Function) where {T<:AbstractFloat, O<:RingOrder}
    ...
    for l in 0:lmax
        W_beam_arr[l+1] = MapGenData.W_beam_fermi(l, PSF_theta)
        next!(p)
    end
...
#%%
# apply_W_beam(map_, lmax, PSF_theta_bini)
@btime W_beam_fermi($lmax, $PSF_theta_bini)
#%%
l=1000
@btime W_beam_fermi(l, PSF_theta_bini)
arg(theta) = 2*pi*sin(theta)*Pl(cos(theta), l)*PSF_theta_bini(theta)
@btime begin
